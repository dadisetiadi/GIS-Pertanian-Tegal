...
<?php
  ...
  <div data-page="about" class="page no-toolbar no-navbar">
    ...
    <?php
        ...
        $jumlah = $_POST['jumlah'];
        mysqli_query($connection,"UPDATE tb_produksipadi_tegaltimur set nama_kelurahan='$nama_kelurahan', luas_panen='$luas_panen',rata_produksi_ha='$rata_produksi_ha',rata_produksi_ton='$rata_produksi_ton', lahan_sawah='$lahan_sawah', lahan_bukan_sawah='$lahan_bukan_sawah', jumlah='$jumlah' where id_kelurahan='$id_kelurahan'");
    ?>
  
  <script type="text/javascript">
    window.alert("Sukses Diubah");
    window.location.href="<?=url('edit_datasawah_tegaltimur')?>";
  </script>
...
if(isset($_GET['hapus'])){
      ...
      mysqli_query($connection,"DELETE from tb_produksipadi_tegaltimur where id_kelurahan='$id_kelurahan'");
}
if(isset($_GET['ubah']) AND isset($_GET['id'])) {
  $id=$_GET['id'];
  include "koneksi.php";
  $data = mysqli_query($connection,"SELECT * from tb_produksipadi_tegaltimur where id_kelurahan='$id'");
  while($d = mysqli_fetch_array($data)){
    $id_kelurahan=  $d['id_kelurahan'];
    $nama_kelurahan = $d['nama_kelurahan'];
    $luas_panen = $d['luas_panen'];
    $rata_produksi_ha = $d['rata_produksi_ha'];
    $rata_produksi_ton = $d['rata_produksi_ton'];
    $lahan_sawah = $d['lahan_sawah'];
    $lahan_bukan_sawah = $d['lahan_bukan_sawah'];
    $jumlah = $d['jumlah'];
  }
  ...
  <div class="container">
    <form method="post" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <a href="<?=url('edit_datasawah_tegaltimur')?>" class="btn btn-danger external"> Kembali</a>
      </div>
    ...
    <div class="page_single layout_fullwidth_padding">          
            <div class="table-responsive">
                <table class="table">
                       ...
                       <?php
                        include "koneksi.php";
                        $query = mysqli_query($connection,"SELECT * FROM tb_produksipadi_tegaltimur ORDER BY id_kelurahan");
                        ?>
                    <thead>
                    <tr>
